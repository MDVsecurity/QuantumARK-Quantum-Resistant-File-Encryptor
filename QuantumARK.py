...
class PasswordDialog:
    ...
    def create_widgets(self):
        ...
        # Título
        title_text = "Nueva Contraseña" if self.is_new_password else "Ingrese Contraseña"
        ...
        if self.is_new_password:
            ...
            if strength < 30:
                if not messagebox.askyesno(
                    "Contraseña Débil", 
                    "La contraseña es débil. ¿Desea continuar?"
                ):
                    return
...
class QRAESFileEncryptorGUI:
    ...
    def create_encrypt_tab(self):
        ...
        file_frame = ttk.LabelFrame(self.encrypt_frame, text="Archivo a Cifrar", padding="10")
        file_frame.pack(fill=tk.X, pady=(0, 20))
        
        self.encrypt_file_var = tk.StringVar()
        file_entry = ttk.Entry(file_frame, textvariable=self.encrypt_file_var, state="readonly")
        ...
        self.encrypt_info_frame = ttk.LabelFrame(self.encrypt_frame, text="Información del Archivo", padding="10")
        self.encrypt_info_frame.pack(fill=tk.X, pady=(0, 20))
        
        self.encrypt_info_text = tk.Text(self.encrypt_info_frame, height=4, state=tk.DISABLED)
        self.encrypt_info_text.pack(fill=tk.X)
        
        # Opciones de cifrado
        options_frame = ttk.LabelFrame(self.encrypt_frame, text="Opciones de Cifrado", padding="10")
    ...
    def create_decrypt_tab(self):
        ...
        # Selección de archivo
        file_frame = ttk.LabelFrame(self.decrypt_frame, text="Archivo a Descifrar", padding="10")
        file_frame.pack(fill=tk.X, pady=(0, 20))
        
        self.decrypt_file_var = tk.StringVar()
        file_entry = ttk.Entry(file_frame, textvariable=self.decrypt_file_var, state="readonly")
        file_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10))
        ...
        self.decrypt_info_frame = ttk.LabelFrame(self.decrypt_frame, text="Información del Archivo Cifrado", padding="10")
        self.decrypt_info_frame.pack(fill=tk.X, pady=(0, 20))
        
        self.decrypt_info_text = tk.Text(self.decrypt_info_frame, height=4, state=tk.DISABLED)
    ...
    def update_decrypt_file_info(self, filename):
        ...
        try:
            if not filename.endswith('.qr256'):
                ...
                self.decrypt_info_text.insert(1.0, "⚠️  El archivo no tiene extensión .qr256")
...
Estado: Listo para descifrar"""
    ...
    def encrypt_file(self):
        ...
        password_dialog = PasswordDialog(self.root, "Nueva Contraseña para Cifrado", True)
        ...
        self.operation_cancelled = False
        thread = threading.Thread(target=self._encrypt_file_thread, args=(filename, password))
        thread.daemon = True
        thread.start()
    ...
    def _encrypt_file_thread(self, filename, password):
        ...
        try:
            ...
            self.root.after(0, lambda: self.set_operation_state(False, "Cifrado completado"))
            self.root.after(0, lambda: messagebox.showinfo(
                "Éxito", 
                f"Archivo cifrado guardado como:\n{output_filename}"
            ))
            
        except Exception as e:
            self.root.after(0, lambda: self.set_operation_state(False, "Error en cifrado"))
            self.root.after(0, lambda: messagebox.showerror("Error", f"Error al cifrar: {str(e)}"))
    ...
    def decrypt_file(self):
        ...
        self.operation_cancelled = False
        thread = threading.Thread(target=self._decrypt_file_thread, args=(filename, password))
        thread.daemon = True
        thread.start()
    ...
    def _decrypt_file_thread(self, filename, password):
        ...
        try:
            self.root.after(0, lambda: self.set_operation_state(True, "Descifrando archivo..."))
            ...
            try:
                ...
                self.root.after(0, lambda: self.set_operation_state(False, "Error: Contraseña incorrecta"))
                self.root.after(0, lambda: messagebox.showerror("Error", "Contraseña incorrecta o archivo corrupto"))
                return
            ...
            try:
                ...
                self.root.after(0, lambda: messagebox.showerror("Error", "Archivo corrupto: metadatos inválidos"))
                return
            
            # Verificar integridad
            actual_checksum = hashlib.sha256(decrypted_data).hexdigest()
            if actual_checksum != metadata.checksum:
                self.root.after(0, lambda: messagebox.showerror("Error", "Error de integridad: archivo corrupto"))
                return
            ...
            self.root.after(0, lambda: self.set_operation_state(False, "Descifrado completado"))
            self.root.after(0, lambda: messagebox.showinfo(
                "Éxito", 
            ...
            self.root.after(0, lambda: self.set_operation_state(False, "Error en descifrado"))
            self.root.after(0, lambda: messagebox.showerror("Error", f"Error al descifrar: {str(e)}"))
    ...
    def batch_encrypt(file_list, password, output_dir=None):
        ...
        for filename in file_list:
            try:
                ...
                results.append({'file': filename, 'output': str(output_filename), 'status': 'success'})
